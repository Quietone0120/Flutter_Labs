...
# ── Har Suld BGM: Mongolian pentatonic minor warrior theme ──────────────────
def make_har_suld_bgm():
    # D minor pentatonic: D F G A C — slow, epic, warrior feel (~76 BPM)
    beat = 0.79

    def tri(freq, duration, sr=SR, vol=0.38, attack=0.06, release=0.18):
        """Triangle wave — warmer, like morin khuur string instrument"""
        n = int(sr * duration)
        atk = int(sr * attack)
        rel = int(sr * release)
        samples = []
        for i in range(n):
            ...
            # Triangle wave
            phase = (freq * t) % 1.0
            s = (4 * phase - 2) if phase < 0.5 else (6 - 4 * phase - 2)
            if i < atk:
                env = i / atk
            elif i > n - rel:
                env = (n - i) / rel
    ...
    # Main melody — pentatonic minor D4, C4, A3, G3, F3, D3
    melody_notes = [
        ('D4', 2), ('A3', 1), ('C4', 1),
        ('G3', 2), ('A3', 2),
        ('F3', 2), ('G3', 1), ('A3', 1),
    ...
    # Low drone bass — power and gravity
    bass_notes = [
        ('D2', 4), ('A2', 4),
        ('D2', 4), ('G2', 4),
        ('D2', 4), ('A2', 4),
    ...
    # Counter melody — higher, breathlike
    counter_notes = [
        ('A4', 4), ('G4', 4),
        ('F4', 4), ('A4', 4),
        ('G4', 2), ('A4', 2), ('C5', 4),
    ...
    mel = []
    for note, beats in melody_notes:
        mel += tri(note_freq(note), beat * beats, vol=0.40)

    bas = []
    for note, beats in bass_notes:
        bas += sine(note_freq(note), beat * beats, vol=0.18)

    cnt = []
    for note, beats in counter_notes:
        cnt += sine(note_freq(note), beat * beats, vol=0.16)

    length = max(len(mel), len(bas), len(cnt))
    mel += [0.0] * (length - len(mel))
    bas += [0.0] * (length - len(bas))
    cnt += [0.0] * (length - len(cnt))
    combined = mix([mel, bas, cnt])
    combined = combined * 2  # loop-ready length
    save_wav('bgm_har_suld.wav', combined)
